...
class MainActivity : AppCompatActivity(), MVPZipCode.ZipcodeView {
    ...
    private fun initCameraButton() {
        binding.addcamera.setOnClickListener {
            ...
            Toast.makeText(this@MainActivity, "Camera button clicked!", Toast.LENGTH_SHORT).show()
        }
    }

    private fun initUploadLayout() {
    ...
    private fun checkThemeAndSetTheme(timeSunset: Long, timeNow: Long) {
        if (timeSunset > timeNow) {
            // it means it is day
            binding.containerMain.setBackgroundResource(R.drawable.last_background)
        } else {
            // it is night
            binding.containerMain.setBackgroundResource(R.drawable.last_background)
        }

    ...
    override fun setResult(forecastResponse: ForecastResponse) {
        ...
        binding.apply {
            forecastResponse.apply {
                val firstForecast = list.firstOrNull()
                firstForecast?.let {
                    txtDegree.text =
                        getString(R.string.feels_like, tempConverter(it.main.feels_like))
                    txtHuimdPercentage.text = it.main.humidity.toString()
                    txtWind.text = "Wind"
                    txtmin.text = tempConverter(it.main.temp_min).toString() + "° Min"
                    txtmax.text = tempConverter(it.main.temp_max).toString() + "° Max"
                }
            }
        }
    }
